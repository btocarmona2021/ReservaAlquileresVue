...
const eliminarComodidad = async (id: number) => {
  if (confirm('¿Estás seguro de que deseas eliminar esta comodidad?')) {
    try {
      ...
      comodidades.value = comodidades.value.filter((comodidad) => comodidad.id !== id);
...
<template>
  <div>
    <div class="addUsuarios">
      <button @click="addcomodidad" class="btn btn-info m-2">Agregar Comodidad</button>
    ...
    <table class="tabla-comodidades">
      ...
      <tbody>
        <tr v-for="comodidad in comodidades" :key="comodidad.id">
          <td>
            <img :src="comodidad.icono" :alt="comodidad.icono" class="tabla-imagen" />
          </td>
          <!--        <td>{{ comodidad.id }}</td>-->
          <td>{{ comodidad.nombre }}</td>
          <td>
            <button @click="actualizarComodidad(comodidad.id)" class="btn btn-sm btn-info">
              Actualizar
            </button>
            <button @click="eliminarComodidad(comodidad.id)" class="btn btn-sm btn-danger m-2">
              Eliminar
            </button>
          </td>
        </tr>
        <tr v-if="comodidades.length === 0">
          <td colspan="4" class="tabla-vacia">No hay comodidades disponibles</td>
        </tr>
      </tbody>
    </table>
  </div>
