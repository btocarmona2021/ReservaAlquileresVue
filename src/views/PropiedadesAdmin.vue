...
const eliminarPropiedad = async (id: number) => {
  if (confirm('¿Estás seguro de que deseas eliminar esta propiedad?')) {
    try {
      ...
      propiedades.value = propiedades.value.filter((propiedad) => propiedad.id !== id);
...
<template>
  <div>
    <div class="addPropiedad">
      <button @click="addpropiedad" class="btn btn-info m-2">Agregar Propiedad</button>
    ...
    <table class="tabla-propiedades">
      ...
      <tbody>
        <tr v-for="propiedad in propiedades" :key="propiedad.id">
          ...
          <td>
            <img :src="propiedad.imagen_principal" :alt="propiedad.titulo" class="tabla-imagen" />
          </td>
          ...
          <td>
            <button @click="addTarifa(propiedad.id)" class="btn btn-info btn-sm">
              Agregar Tarifa
            </button>
            <button @click="eliminarPropiedad(propiedad.id)" class="btn btn-danger btn-sm m-2">
              Eliminar Propiedad
            </button>
          </td>
        </tr>
        <tr v-if="propiedades.length === 0">
          <td colspan="9" class="tabla-vacia">No hay propiedades disponibles</td>
        </tr>
      </tbody>
    </table>
  </div>
