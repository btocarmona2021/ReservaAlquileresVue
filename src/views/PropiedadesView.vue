...
<template>
  <div class="container py-5">
      ...
      <div class="row g-4">
        ...
        >
          <div class="card shadow-lg border-0 rounded overflow-hidden">
            ...
            />
            <div class="card-body">
              <h5 class="card-title text-dark font-weight-bold">{{ propiedad.titulo }}</h5>
              <p class="card-text text-muted">{{ propiedad.descripcion }}</p>
              <p class="text-muted mb-0">Ubicación: <strong>{{ propiedad.ubicacion }}</strong></p>
              <div class="d-flex justify-content-between align-items-center mt-3">
                <p class="mb-0 text-success">
                  <strong>{{ propiedad.precio_por_noche }} por día</strong>
                </p>
                <p class="mb-0 text-secondary">Capacidad: {{ propiedad.capacidad_personas }} Personas</p>
              </div>
            </div>
    ...
    <div v-else>
      <p class="text-center text-danger">No hay propiedades disponibles en este momento.</p>
    </div>
  </div>
</template>
