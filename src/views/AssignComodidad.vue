...
<template>
  <div class="contenedor">
    ...
    <select class="form-control w-75" v-model="propiedad_id" id="propiedad_id">
      <option v-for="propiedad in propiedades" :value="propiedad.id">{{ propiedad.titulo }}</option>
    </select>

    <label for="comodidad_id">Seleccione una comodidad</label>
    <select class="form-control w-75" v-model="comodidad_id" id="comodidad_id">
      <option v-for="comodidad in comodidades" :value="comodidad.id">{{ comodidad.nombre }}</option>
    </select>
    <p
      :class="{
        correcto: errorEncontrado == 'Comodidad asignada correctamente a la propiedad',
